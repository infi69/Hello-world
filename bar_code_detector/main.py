...
def run_pipeline(
    ...
    for bi, batch in enumerate(batches, start=1):
        ...
        # Stable order within batch by filename
        batch_rows.sort(key=lambda r: r["filename"])
        ...
        logger.info(
            ...
            sum(1 for r in all_rows if r["decoded"]),
            len(all_rows),
        )
    ...
    for r in all_rows:
        print(
            f"{r['filename']:<42} {str(r['decoded']):<5} {(r.get('method') or ''):<22} "
